...
This package provides model selection in the Web GUI: the `/model` popup command and the composer's model seat, both over one per-session directory of provider-grouped models. Choosing a model submits the complete selection — provider, model, and reasoning effort — which the Host snapshots at the next prompt-assembly boundary, so the following request uses it while a running step keeps its assembled selection. The composer seat shows a two-level Model/Effort menu: models stay provider-grouped, and the Effort row opens a slider over the selected exact model's adapter-owned levels, whose filled track and glow scale with the selected level. When the Host reports that no adapter serves the session's route, the composer input goes inert until a route becomes available.
...
Models stay grouped by provider. The effort row opens a slider over the model's advertised levels, weakest first, and shows the selected level's name and description; the model list keeps names only. The `/model` popup applies the selected model's default effort; the composer can then slide to any advertised effort, and a model whose adapter declares no default keeps a Default chip beside the slider. An adapter without reasoning metadata leaves the Effort row absent; there is no arbitrary effort input.
